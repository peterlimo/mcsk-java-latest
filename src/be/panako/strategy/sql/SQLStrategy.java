...
import be.panako.http.PanakoWebserviceClientSql;
import be.panako.strategy.QueryResult;
import be.panako.strategy.QueryResultHandler;
import be.panako.strategy.SerializedFingerprintsHandler;
import be.panako.strategy.Strategy;
import be.panako.strategy.sql.storage.SQLFingerprintHit;
import be.panako.strategy.sql.storage.SQLFingerprintQueryMatch;
import be.panako.strategy.sql.storage.SQLDBStorage;
import be.panako.strategy.sql.storage.Storage;
...
public class SQLStrategy extends Strategy {
	
	private final static Logger LOG = Logger.getLogger(SQLStrategy.class.getName());
	
	private final Storage storage;
	
	public SQLStrategy(){
		storage = SQLDBStorage.getInstance();
	}
	...
	public double store(String resource, String description) {
		...
		final SQLEventPointProcessor minMaxProcessor = new SQLEventPointProcessor(size,overlap,samplerate);
		d.addAudioProcessor(minMaxProcessor);
		d.run();
		Set<SQLFingerprint> fingerprints = new HashSet<SQLFingerprint>(minMaxProcessor.getFingerprints());
		
		int identifier = FileUtils.getIdentifier(resource);
		
		
		
		//ArrayList<int[]> arrayFIngerprints = new ArrayList<int [] >();
		 HashSet<int []> fftFingerprintStore = new  HashSet<int []>();
		for(SQLFingerprint fingerprint: fingerprints){
			int[] value = {  fingerprint.hash(), fingerprint.t1, identifier };
			//storage.addFingerprint(identifier, fingerprint.t1, fingerprint.hash());
			fftFingerprintStore.add(value);
				}
		storage.addFingerprint(fftFingerprintStore);
	...
	public List<SQLFingerprint> extractFingerprintsFromQuery(String query){
		int samplerate = Config.getInt(Key.NFFT_SAMPLE_RATE);
		int size = Config.getInt(Key.NFFT_SIZE);
		int overlap = size - Config.getInt(Key.NFFT_STEP_SIZE);
		...
		d.addAudioProcessor(minMaxProcessor);
		d.run();
		List<SQLFingerprint> fingerprints = new ArrayList<SQLFingerprint>(minMaxProcessor.getFingerprints());
		return fingerprints;
	}
	...
	public void query(String query, int maxNumberOfResults,Set<Integer> avoid,
		...
		final List<SQLFingerprintQueryMatch> queryMatches = new ArrayList<SQLFingerprintQueryMatch>();
		...
		if(queryMatches.isEmpty()){
			...
			for(SQLFingerprintQueryMatch match : queryMatches){
	...
	private void processMonitorQueryToSerializeFingerprints(float[] audioBuffer,SerializedFingerprintsHandler handler,double queryOffset){
		...
		try {
			...
			double queryDuration = d.secondsProcessed();
			List<SQLFingerprint> fingerprints = new ArrayList<SQLFingerprint>(minMaxProcessor.getFingerprints());
			handler.handleSerializedFingerprints(PanakoWebserviceClientSql.serializeFingerprintsToJson(fingerprints),queryDuration,queryOffset);
	...
	public void matchSerializedFingerprints(String serizalizedFingerprints,final  int maxNumberOfResults,
		...
		List<SQLFingerprint> fingerprints = PanakoWebserviceClientSql.deserializeFingerprintsFromJson(serizalizedFingerprints);
		final List<SQLFingerprintQueryMatch> queryMatches = new ArrayList<SQLFingerprintQueryMatch>();
	...
	public boolean isStorageAvailable() {
		return SQLDBStorage.getInstance() != null;
	}
	...
	public void compareFingerprints(File reference,File other) {
		...
		SQLEventPointProcessor minMaxProcessor = new SQLEventPointProcessor(size,overlap,samplerate);
		d.addAudioProcessor(minMaxProcessor);
		d.run();
		List<SQLFingerprint> fingerprintsReference = minMaxProcessor.getFingerprints();
		
		final List<SQLFingerprint> fingerprintsQuery;		
		if(reference.getAbsolutePath().equals(other.getAbsolutePath())){
			...
			minMaxProcessor = new SQLEventPointProcessor(size,overlap,samplerate);
			d.addAudioProcessor(minMaxProcessor);
			d.run();
		...
		TreeMap<Integer,List<SQLFingerprint>> queryPrintsOrderedByTime;
		queryPrintsOrderedByTime = new TreeMap<>();
		for(SQLFingerprint fingerprint: fingerprintsQuery){
			...
			if( ! queryPrintsOrderedByTime.containsKey(milliseconds)){
				queryPrintsOrderedByTime.put(milliseconds,new ArrayList<SQLFingerprint>());
			}
			queryPrintsOrderedByTime.get(milliseconds).add(fingerprint);
		}
		
		TreeMap<Integer,List<SQLFingerprint>> referencePrintsOrderedByTime;
		referencePrintsOrderedByTime = new TreeMap<>();
		for(SQLFingerprint fingerprint: fingerprintsReference){
			int milliseconds = fingerprint.t1 * samplerate / Config.getInt(Key.NFFT_STEP_SIZE);
			if( ! referencePrintsOrderedByTime.containsKey(milliseconds)){
				referencePrintsOrderedByTime.put(milliseconds,new ArrayList<SQLFingerprint>());
			}
		...
		for(Map.Entry<Integer,List<SQLFingerprint>> entry : queryPrintsOrderedByTime.entrySet()){
			int currentMilliseconds = entry.getKey();
			
			SortedMap<Integer,List<SQLFingerprint>> searchSpace = referencePrintsOrderedByTime.subMap(currentMilliseconds, maxMilliseconds);
			//SortedMap<Integer,List<NFFTFingerprint>> searchSpace = referencePrintsOrderedByTime.subMap(0, maxMilliseconds);
			SortedMap<Integer,List<SQLFingerprint>> query = queryPrintsOrderedByTime.subMap(currentMilliseconds, currentMilliseconds+windowInMilliseconds);
	...
	private Set<Integer> search(SortedMap<Integer, List<SQLFingerprint>> searchSpace,
			SortedMap<Integer, List<SQLFingerprint>> query) {
		
		Set<Integer> millisecondMatches = new HashSet<Integer>();
		//create a database
		HashMap<Integer,List<SQLFingerprint>> database = new HashMap<>();
		for(Map.Entry<Integer,List<SQLFingerprint>> entry : searchSpace.entrySet()){
			for(SQLFingerprint print : entry.getValue()){
				int hash = print.hash();
				if(!database.containsKey(hash)){
					database.put(hash, new ArrayList<SQLFingerprint>());
				}
		...
		List<SQLFingerprintHit> matches = new ArrayList<>();
		for(Map.Entry<Integer,List<SQLFingerprint>> entry : query.entrySet()){
			for(SQLFingerprint queryPrint : entry.getValue()){
				int hash = queryPrint.hash();
				if(database.containsKey(hash)){
					List<SQLFingerprint> dbPrints = database.get(hash);
					for(SQLFingerprint dbPrint : dbPrints ){
						SQLFingerprintHit hit = new SQLFingerprintHit();
		...
		for(SQLFingerprintHit hit : matches){
	...
	public SQLStreamSync sync(String reference,String other){
		SQLStreamSync syncObject = new SQLStreamSync(reference, other);
