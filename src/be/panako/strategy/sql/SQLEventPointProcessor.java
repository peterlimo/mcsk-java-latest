...
public class SQLEventPointProcessor implements AudioProcessor {
	...
	private final List<SQLEventPoint> eventPoints = new ArrayList<>();
	private final List<SQLFingerprint> fingerprints = new ArrayList<>();
	...
	public SQLEventPointProcessor(int size,int overlap,int sampleRate){
		this(size,overlap,sampleRate,defaultMaxFilterWindowSize,defaultMinFilterWindowSize);
	}
	
	private SQLEventPointProcessor(int fftSize,int overlap,int sampleRate, int maxFilterWindowSize,int minFilterWindowSize){
	...
	private void horizontalFilter() {
		...
		for(int i = 0 ; i<frameMagnitudes.length ; i++){
			...
			if(currentVal == maxVal && currentVal !=0 && minVal != 0 ){
				...
				eventPoints.add(new SQLEventPoint(frameUnderAnalysis+longestFilterWindowSize, i, frequencyEstimate, currentVal,currentVal) );
				filterEventPoints();
			}
		}
	}
	...
	private void filterEventPoints(){
		...
		HashMap<Integer,ArrayList<SQLEventPoint>> counter =  new HashMap<Integer,ArrayList<SQLEventPoint>>();
		for(int i = Math.max(eventPoints.size()-history, 0) ; i< eventPoints.size();i++){
			SQLEventPoint ep  = eventPoints.get(i);
			if(!counter.containsKey(ep.t)){
				counter.put(ep.t, new ArrayList<SQLEventPoint>());
			}
			counter.get(ep.t).add(ep);
		}
		for(Integer frameIndex : counter.keySet()){
			ArrayList<SQLEventPoint> points = counter.get(frameIndex);
			if(points.size()>maxEventPointsPerFrame){
				Collections.sort(points,new Comparator<SQLEventPoint>() {
					@Override
					public int compare(SQLEventPoint o1, SQLEventPoint o2) {
		...
		List<SQLEventPoint> pointsToDelete = new ArrayList<SQLEventPoint>();
		for(int i = Math.max(eventPoints.size()-history, 0) ; i< eventPoints.size();i++){
			
			SQLEventPoint first  = eventPoints.get(i);
			for(int j  = i+1  ; j< eventPoints.size();j++){
				SQLEventPoint other  = eventPoints.get(j);
		...
		for(SQLEventPoint pointToDelete : pointsToDelete){
	...
	public List<SQLFingerprint> getFingerprints(){
		return fingerprints;
	}
	...
	private void packEventPointsIntoFingerprints(){
		...
		TreeMap<Float,SQLFingerprint> printsOrderedByEnergy = new TreeMap<Float,SQLFingerprint>();
		...
		for(int i = 0; i < eventPoints.size();i++){
			...
			for(int j = 0; j < eventPoints.size() ;j++){
				...
				if(t1 < t2 && f1 != f2 &&  Math.abs(t2-t1) * frameDurationInMS > minEventPointDeltaTInMs &&  Math.abs(t2-t1) * frameDurationInMS < maxEventPointDeltaTInMs && f2 > minfFirstLevel && f2 < maxfFirstLevel){
					...
					fingerprint = new SQLFingerprint(eventPoints.get(i),eventPoints.get(j));
		...
		HashMap<SQLEventPoint,Integer> printsPerPoint = new HashMap<SQLEventPoint, Integer>();
		for(int i = 0; i < eventPoints.size();i++){
			printsPerPoint.put(eventPoints.get(i), 0);
		}
		for(Float key: printsOrderedByEnergy.descendingKeySet()){
			SQLFingerprint print = printsOrderedByEnergy.get(key);
	...
	public List<SQLEventPoint> getEventPoints() {
		return eventPoints;
	}
