...
public class SQLStreamSync {

	private static String prevReference;
	private static List<SQLFingerprint> prevReferencePrints;

	private final String reference;
	private final String other;

	private SQLSyncMatch match;
	...
	public SQLStreamSync(String reference, String other) {
		this.reference = reference;
		this.other = other;
		match = null;
	}

	private List<SQLFingerprint> extractFingerprints(String resource) {
		...
		d.run();
		return new ArrayList<SQLFingerprint>(minMaxProcessor.getFingerprints());
	}

	public void synchronize() {
		// extract fingerprints for reference audio
		// if needed, the reference prints are cached.
		List<SQLFingerprint> referencePrints;
		...
		List<SQLFingerprint> extracted = extractFingerprints(other);
		filterPrints(extracted);
		
		match(referencePrints, extracted);
	}

	private void filterPrints(List<SQLFingerprint> prints) {
		...
		for (int i = 0; i < prints.size(); i++) {
			SQLFingerprint print = prints.get(i);
	...
	private void match(List<SQLFingerprint> referencePrints,List<SQLFingerprint> otherPrints) {
		// create a map with the fingerprint hash as key, and fingerprint object
		// as value.
		// Warning: only a single object is kept for each hash.
		HashMap<Integer, SQLFingerprint> referenceHash = fingerprintsToHash(referencePrints);

		HashMap<Integer, SQLFingerprint> otherHash = fingerprintsToHash(otherPrints);
		printOffset(referenceHash, otherHash);		
	}

	private void printOffset(HashMap<Integer, SQLFingerprint> referenceHash,
			HashMap<Integer, SQLFingerprint> otherHash) {
		// key is the offset, value a list of fingerprint objects.
		HashMap<Integer, List<SQLFingerprint>> mostPopularOffsets = new HashMap<Integer, List<SQLFingerprint>>();
		
		int minimumAlignedMatchesThreshold = Config.getInt(Key.SYNC_MIN_ALIGNED_MATCHES);
		int maxAlignedOffsets = 0;

		List<SQLFingerprint> matchingPairs = null;
		int bestOffset = -1;

		int numberOfMatchingHashes = 0;
		// iterate each fingerprint in the reference stream
		for (Map.Entry<Integer, SQLFingerprint> entry : referenceHash.entrySet()) {
			// if the fingerprint is also present in the other stream
			if (otherHash.containsKey(entry.getKey())) {

				SQLFingerprint referenceFingerprint = entry.getValue();
				SQLFingerprint otherFingerprint = otherHash.get(entry.getKey());
				...
				if (!mostPopularOffsets.containsKey(offset)) {
					mostPopularOffsets.put(offset, new ArrayList<SQLFingerprint>());
				}
		...
		if (maxAlignedOffsets >= minimumAlignedMatchesThreshold) {
			...
			for (int i = 0; i < matchingPairs.size(); i += 2) {
				SQLFingerprint refFingerprint = matchingPairs.get(i);
				SQLFingerprint otherFingerprint = matchingPairs.get(i + 1);
			...
			SQLSyncMatch match = new SQLSyncMatch(reference, other,
	...
	private HashMap<Integer, SQLFingerprint> fingerprintsToHash(List<SQLFingerprint> fingerprints) {
		HashMap<Integer, SQLFingerprint> hash = new HashMap<>();
